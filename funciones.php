        ...
        {
             return $dbconn = $conectate=pg_connect("host='www.smarthub.design' port=5432 dbname='aguara' user='postgres' password='Riveros200587!'")or die ('Error al conectar a la base de datos');
        } 
       ...
       function func_existeDatoDetalle($dato1,$dato2 ,$tabla, $columna1,$columna2, $database){
        ...
        function obtenerCodigoPersona($IMEI){
            $query = pg_query("select per_cod from personas where per_imei='$IMEI' and per_estado='t'");
            $row1 = pg_fetch_array($query);
            return $row1[0];
        }
          //funcion para comprobar si existe el mismo dato en la tabla
